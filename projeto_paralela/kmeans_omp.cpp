...
public:
  ...
  void run(vector<Point> &points) {
    ...
    while (true) {
      ...
      // Limpa os pontos de cada cluster antes de nova atribuição
      for (int i = 0; i < K; i++) {
        clusters[i].clearPoints();
      }

      // Vetor auxiliar para atribuição dos clusters
      vector<int> new_assignments(total_points);

// Paraleliza apenas a atribuição dos clusters
#pragma omp parallel for schedule(static)
      for (int i = 0; i < total_points; i++) {
        int id_nearest_center = getIDNearestCenter(points[i]);
        new_assignments[i] = id_nearest_center;
      }

      // Atualiza clusters sequencialmente
      for (int i = 0; i < total_points; i++) {
        int id_old_cluster = points[i].getCluster();
        int id_new_cluster = new_assignments[i];
        if (id_old_cluster != id_new_cluster) {
          done = false;
        }
...
// Paraleliza o cálculo dos centróides
#pragma omp parallel for schedule(static)
      for (int i = 0; i < K; i++) {
        for (int j = 0; j < total_values; j++) {
          double sum = 0.0;
